...
export default function Clients() {
  ...
  const load = () => { setLoading(true); get<Client[]>('/api/clients').then(setItems).catch(e => setError(e.message)).finally(() => setLoading(false)); };
  useEffect(load, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (modal === 'create') await post('/api/clients', form);
      else if (editing) await put(`/api/clients/${editing.id}`, form);
      setModal(null); load();
      ...
      <div className="glass-card">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/5">{['Company','Contact','Email','Phone','Status',''].map(h => <th key={h} className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider text-left">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-white/5">
              {loading ? <tr><td colSpan={6} className="px-5 py-10 text-center text-slate-500">Loading…</td></tr>
              : items.length === 0 ? <tr><td colSpan={6} className="px-5 py-12 text-center"><Building2 className="h-10 w-10 text-slate-600 mx-auto mb-2" /><p className="text-slate-400">No clients yet</p></td></tr>
              : items.map(c => (
                <tr key={c.id} className="table-row-hover">
                  ...
                  <td className="px-5 py-3"><button onClick={() => openEdit(c)} className="p-1.5 text-slate-400 hover:text-white hover:bg-white/10 rounded-md transition-colors"><Pencil className="h-4 w-4" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 glass-card w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-white">{modal === 'create' ? 'Add Client' : 'Edit Client'}</h2>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type ?? 'text'} required={f.required} value={(form as Record<string,string>)[f.key]} onChange={e => setForm(prev => ({...prev, [f.key]: e.target.value}))} className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-none focus:ring-1 focus:ring-blue-500" />
                </div>
              ))}
